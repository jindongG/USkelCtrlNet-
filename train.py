...
class TrainManager:
    ...
    def get_model(self, model_dct):
        name = model_dct["name"]
        if name == "SwinSnake_Alter":
            model = SwinSnake_Alter(
                img_ch=3,
                output_ch=1,
                layer_depth=model_dct["layer_depth"],
                kernel_size=model_dct["kernel_size"],
                extend_scope=model_dct["extend_scope"],
                down_layer=model_dct["down_layer"],
                rate=model_dct["rate"],
                repeat_n=model_dct["repeat_n"],
        ...
        elif model_dct["name"] == "OCTAMamba":
            model = OCTAMamba(in_channels=3).to(device)
            # 注意：OCTAMamba 自带 sigmoid 输出，不要再 ModifiedModel 包一层
        else:
